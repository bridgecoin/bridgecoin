...
#else
    const char* pszModule = "bridgecoin";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "bridgecoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No bridgecoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override bridgecoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "bridgecoin.pid"));
